...
async function connect(){
  if(global.db) return global.db;
    const conn = await MongoClient.connect("mongodb+srv://mikael:123@cluster0.gvgsp.mongodb.net/");
...
router.get("/user/:id?", async function (req, res, next) {
  try {
    ...
    else res.json(await db.collection("user").find().toArray());
  } catch (ex) {
    console.log(ex);
...
router.post("/login", async function (req, res, next) {
  try {
    ...
    const token = jwt.sign(
      { id: userAuth._id },
      "def5eb50dca1270a015c1ac2a8569fb69d0584d5fbd76f45e6f706517be1e63d",
      { expiresIn: "1h" }
    );
    res.json({ token });
  } catch (ex) {
